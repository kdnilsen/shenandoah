...
#ifdef ASSERT
    assert(!ShenandoahThreadLocalData::thread_is_marked_as_suspendible(Thread::current()), "Don't rejoin STS");
...
#ifdef ASSERT
    assert(ShenandoahThreadLocalData::thread_is_marked_as_suspendible(Thread::current()), "Should be STS at exit");
